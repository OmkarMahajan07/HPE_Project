...
func (server *FTPServer) handleClient(conn net.Conn) {
	...
	logger.Printf("Client connected from IP: %s", clientIP)
	...
	session.logger.Printf("Client connected")
	...
	for scanner.Scan() {
		...
		session.logger.Printf("Command received: %s", command)
		session.handleCommand(command)
	}

	if err := scanner.Err(); err != nil {
		session.logger.Printf("Connection error: %v", err)
	}

	session.logger.Printf("Client disconnected")
}
...
func (session *ClientSession) handlePASS(password string) {
	...
	if userProfile.Password == password || (session.username == "anonymous" && password != "") {
		...
		session.logger.Printf("User %s authenticated successfully", session.username)
	} else {
		...
		if attempts >= 5 {
			...
			session.logger.Printf("User %s locked after %d failed attempts", session.username, attempts)
		}
...
func (session *ClientSession) handleCWD(path string) {
	...
	if stat, err := os.Stat(fullPath); err == nil && stat.IsDir() {
		...
		session.logger.Printf("Changed directory to: %s", newPath)
	} else {
...
func (session *ClientSession) handleLIST(path string) {
	...
	session.logger.Printf("Directory listing sent for: %s", listPath)
}
...
func (session *ClientSession) handleRETR(filename string) {
	...
	if session.restartPos > 0 {
		...
		session.logger.Printf("Resume transfer from position %d", session.restartPos)
	}
	...
	session.logger.Printf("File sent: %s (%d bytes, %.2f MB/s pure transfer, %v lock wait) - %s", 
		filename, totalWritten, timingReport.TransferSpeed, 
		timingReport.LockWaitTime.Round(time.Millisecond), timingReport.String())
}
func (session *ClientSession) handleSTOR(filename string) {
	...
	if session.restartPos > 0 {
		...
		session.logger.Printf("Resume upload from position %d", session.restartPos)
		} else {
			// Normal mode - create new file with READ-WRITE permissions for memory mapping
			// Use O_RDWR for memory mapping compatibility
	...
	// PERFORMANCE FIX: Use memory-mapped I/O for large files for maximum performance
	// This provides true zero-copy transfers matching the client-side implementation
	var bytesWritten int64
	var uploadErr error
	
	// Get file info to check size and determine optimal approach
	fileInfo, statErr := file.Stat()
	if statErr != nil {
		session.sendResponse(550, "Failed to get file statistics")
		return
	}
	
	// For large files (>100MB), use memory-mapped I/O for optimal performance
	// For smaller files, use buffered approach to avoid mapping overhead
	if fileInfo.Size() >= MmapThreshold || session.restartPos >= MmapThreshold {
		// Use memory-mapped upload for large files
		bytesWritten, uploadErr = session.memoryMappedUpload(file, reader)
		if uploadErr != nil {
			// Fallback to buffered approach if memory mapping fails
			session.logger.Printf("Memory mapping failed (%v), falling back to buffered upload", uploadErr)
			bytesWritten, uploadErr = session.ultraOptimizedBufferedUpload(file, reader)
		}
	} else {
		// Use optimized buffered approach for smaller files
		bytesWritten, uploadErr = session.ultraOptimizedBufferedUpload(file, reader)
	}
	...
	session.sendResponse(226, "Transfer complete")
	session.logger.Printf("File received: %s (%d bytes, %.2f MB/s pure transfer, %v lock wait) - %s", 
		filename, bytesWritten, timingReport.TransferSpeed, 
		timingReport.LockWaitTime.Round(time.Millisecond), timingReport.String())
...
func (session *ClientSession) handleFEAT() {
	...
	session.logger.Printf("FEAT command processed - advertised server capabilities")
}
	...
	case "UTF8":
		...
		if len(parts) >= 2 {
			...
			case "ON":
				...
				session.logger.Printf("UTF-8 support enabled")
			case "OFF":
				session.utf8Enabled = false
				session.sendResponse(200, "UTF8 set to off")
				session.logger.Printf("UTF-8 support disabled")
			default:
...
func (session *ClientSession) handleREST(position string) {
	...
	session.logger.Printf("REST command: restart position set to %d", pos)
}
...
func (session *ClientSession) handleALLO(args string) {
	...
	// For compatibility, always respond success (storage allocation is automatic)
...
func (session *ClientSession) handleAPPE(filename string) {
	...
	session.logger.Printf("File appended: %s (%d bytes, %.2f MB/s pure transfer, %v lock wait) - %s", 
		filename, bytesWritten, timingReport.TransferSpeed, 
		timingReport.LockWaitTime.Round(time.Millisecond), timingReport.String())
}
...
func (session *ClientSession) handleSTAT(args string) {
	...
	session.logger.Printf("STAT command processed for: %s", args)
}
...
func (session *ClientSession) optimizedMmapUpload(dst *os.File, src io.Reader) (int64, error) {
	// Pre-allocate file space for memory mapping - start with reasonable size
	initialSize := int64(256 * 1024 * 1024) // 256MB initial allocation (more conservative)
	err := dst.Truncate(initialSize)
	if err != nil {
		// If we can't pre-allocate, fall back to streaming
		session.logger.Printf("Failed to pre-allocate file space (%d MB), falling back to streaming: %v", initialSize/(1024*1024), err)
	...
	// Use smaller chunk size to reduce network timeout risk
	chunkSize := 16 * 1024 * 1024 // 16MB chunks for reliable transfer
	var totalWritten int64

	// Add progress logging for large files
	lastLoggedMB := int64(0)
	logIntervalMB := int64(100) // Log every 100MB
	...
	for {
		...
		// If we need more space, expand the mapping more conservatively
		if remainingSpace < int64(chunkSize) {
			// Expand the file size more conservatively to avoid huge allocations
			currentSize := int64(len(mmapFile.data))
			// Increase by 256MB instead of doubling
			newSize := currentSize + 256*1024*1024
			if newSize-totalWritten < int64(chunkSize) {
				newSize = totalWritten + int64(chunkSize)*4 // Ensure enough space
			}
			
			// Close current mapping before expanding
			mmapFile.Close()
			
			// Log expansion for monitoring
			session.logger.Printf("Expanding memory-mapped file from %d MB to %d MB", 
				currentSize/(1024*1024), newSize/(1024*1024))

			err := dst.Truncate(newSize)
			if err != nil {
				// If truncate fails, fall back to streaming
				session.logger.Printf("Failed to expand file, falling back to streaming: %v", err)
				return session.fallbackStreamingUpload(dst, src)
			}

			mmapFile, err = session.createWritableMmapFile(dst, newSize)
			if err != nil {
				// If remapping fails, fall back to streaming
				session.logger.Printf("Failed to remap file, falling back to streaming: %v", err)
				return session.fallbackStreamingUpload(dst, src)
			}
			remainingSpace = int64(len(mmapFile.data)) - totalWritten
		}
		...
		if n > 0 {
			...
			currentMB := totalWritten / (1024 * 1024)
			if currentMB >= lastLoggedMB + logIntervalMB {
				session.logger.Printf("Upload progress: %d MB written", currentMB)
...
func (session *ClientSession) createWritableMmapFile(file *os.File, size int64) (*MmapFile, error) {
	...
	// Debug: Log file information for diagnosis
	fileInfo, _ := file.Stat()
	session.logger.Printf("Debug: Creating writable mmap for file %s, fd=%d, size=%d, mode=%v", 
		file.Name(), fd, size, fileInfo.Mode())
...
// memoryMappedUpload implements true memory-mapped upload to match download performance
// This uses direct memory mapping to write incoming data with zero-copy transfers
func (session *ClientSession) memoryMappedUpload(file *os.File, reader io.Reader) (int64, error) {
	// First, we need to determine the expected file size for memory mapping
	// Read the first chunk to estimate or get size info
	initialBuffer := make([]byte, 64*1024) // 64KB initial buffer
	n, err := reader.Read(initialBuffer)
	if err != nil && err != io.EOF {
		return 0, fmt.Errorf("failed to read initial data: %v", err)
	}
	...
	// Start with initial data size and expand as needed
	expectedSize := int64(n)
	if err != io.EOF {
		// Estimate larger size for streaming data - start with 1GB
		expectedSize = 1024 * 1024 * 1024 // 1GB initial allocation
	}
	
	// Create memory-mapped file for writing (like download uses for reading)
	mmapFile, mmapErr := session.createExpandableMmapFile(file, expectedSize)
	if mmapErr != nil {
		return 0, fmt.Errorf("failed to create memory-mapped file: %v", mmapErr)
	}
	defer session.closeMmapFile(mmapFile)
	
	// Write initial data to memory-mapped region
	copy(mmapFile.data[:n], initialBuffer[:n])
	totalWritten := int64(n)
	
	// Stream data directly into memory-mapped region (NO intermediate buffer)
	for err != io.EOF {
		// Check if we need to expand the mapping
		if totalWritten >= mmapFile.size-64*1024 { // Leave 64KB buffer
			newSize := mmapFile.size * 2 // Double the size
			if expandErr := session.expandMmapFile(mmapFile, newSize); expandErr != nil {
				session.logger.Printf("Failed to expand memory-mapped file: %v", expandErr)
				// Continue with current size
			}
		}
		
		// Read directly into memory-mapped region (ZERO COPY)
		remaining := mmapFile.size - totalWritten
		if remaining <= 0 {
			break // No more space
		}
		...
		n, err = reader.Read(mmapFile.data[totalWritten:totalWritten+chunkSize])
		if n > 0 {
			totalWritten += int64(n)
		}
		
		if err != nil && err != io.EOF {
			return totalWritten, fmt.Errorf("read error during memory-mapped upload: %v", err)
		}
	}
	
	// Truncate file to actual size written
	if truncateErr := file.Truncate(totalWritten); truncateErr != nil {
		session.logger.Printf("Warning: failed to truncate file to actual size: %v", truncateErr)
	}
	
	// Flush memory-mapped data to disk
	if flushErr := session.flushMmapFile(mmapFile); flushErr != nil {
		session.logger.Printf("Warning: failed to flush memory-mapped data: %v", flushErr)
	}
	
	return totalWritten, nil
}

// createExpandableMmapFile creates a memory-mapped file that can be expanded during upload
func (session *ClientSession) createExpandableMmapFile(file *os.File, initialSize int64) (*MmapFile, error) {
	// Ensure file is large enough for initial mapping
	if truncateErr := file.Truncate(initialSize); truncateErr != nil {
		return nil, fmt.Errorf("failed to set initial file size: %v", truncateErr)
	}
	
	return session.createWritableMmapFile(file, initialSize)
}

// expandMmapFile expands an existing memory-mapped file to a new size
func (session *ClientSession) expandMmapFile(mmapFile *MmapFile, newSize int64) error {
	// Close current mapping
	if err := session.closeMmapFileMapping(mmapFile); err != nil {
		session.logger.Printf("Warning: failed to close existing mapping during expansion: %v", err)
	}
	
	// Expand the underlying file
	if err := mmapFile.file.Truncate(newSize); err != nil {
		return fmt.Errorf("failed to expand file: %v", err)
	}
	
	// Create new mapping with expanded size
	newMmap, err := session.createWritableMmapFile(mmapFile.file, newSize)
	if err != nil {
		return fmt.Errorf("failed to create expanded mapping: %v", err)
	}
	
	// Update the mmap file structure
	mmapFile.data = newMmap.data
	mmapFile.size = newSize
	mmapFile.handle = newMmap.handle
	
	session.logger.Printf("Expanding memory-mapped file from %d MB to %d MB", 
		mmapFile.size/(1024*1024), newSize/(1024*1024))
	
	return nil
}

// closeMmapFileMapping closes only the memory mapping, not the file
func (session *ClientSession) closeMmapFileMapping(mmapFile *MmapFile) error {
	if mmapFile.handle != 0 {
		if err := syscall.UnmapViewOfFile(uintptr(unsafe.Pointer(&mmapFile.data[0]))); err != nil {
			return fmt.Errorf("failed to unmap view: %v", err)
		}
		if err := syscall.CloseHandle(mmapFile.handle); err != nil {
			return fmt.Errorf("failed to close mapping handle: %v", err)
		}
		mmapFile.handle = 0
	}
	return nil
...
// flushMmapFile ensures memory-mapped data is written to disk
func (session *ClientSession) flushMmapFile(mmapFile *MmapFile) error {
	// Flush the memory-mapped region to disk
	if len(mmapFile.data) > 0 {
		// Use Windows FlushViewOfFile to ensure data is written
		kernel32 := syscall.NewLazyDLL("kernel32.dll")
		flushViewOfFile := kernel32.NewProc("FlushViewOfFile")
		
		ret, _, err := flushViewOfFile.Call(
			uintptr(unsafe.Pointer(&mmapFile.data[0])),
			uintptr(len(mmapFile.data)),
		)
		if ret == 0 {
	...
	if syncErr := mmapFile.file.Sync(); syncErr != nil {
		return fmt.Errorf("file sync failed: %v", syncErr)
	}
	
	return nil
}

// closeMmapFile properly closes a memory-mapped file
func (session *ClientSession) closeMmapFile(mmapFile *MmapFile) {
	if mmapFile == nil {
		return
	}
	...
	if err := session.closeMmapFileMapping(mmapFile); err != nil {
		session.logger.Printf("Warning: failed to close memory mapping: %v", err)
	}
}
