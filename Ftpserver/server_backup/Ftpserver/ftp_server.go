...
func (session *ClientSession) handleRETR(filename string) {
	fullPath := filepath.Join(session.rootDir, filename)
	file, err := os.Open(fullPath)
	if err != nil {
		session.sendResponse(550, "Failed to open file")
		return
	}
	defer file.Close()
	...
	if err != nil {
		session.sendResponse(550, "Failed to stat file")
		return
	}
	...
	defer session.closeDataConnection() // will close after transfer completes

	session.sendResponse(150, fmt.Sprintf("Opening binary mode data connection for %s (%d bytes)", filename, fileSize))

	// Buffered writer wraps connection to ensure flush
	bufWriter := bufio.NewWriterSize(dataConn, 32*1024*1024) // 32MB buffer
	var writer io.Writer = bufWriter
	var zWriter *zlib.Writer

	if session.mode == "Z" {
		zWriter = zlib.NewWriter(bufWriter)
		writer = zWriter
		defer func() {
			zWriter.Flush()
			zWriter.Close()
	...
	mmapFile, mmapErr := openMmapFile(file)
	if mmapErr == nil && mmapFile != nil {
		defer mmapFile.Close()
		transferData = mmapFile.data
	} else {
		if fileSize > 500*1024*1024 {
			session.sendResponse(550, "File too large for memory fallback")
			return
		}
		transferData, err = io.ReadAll(file)
		if err != nil {
			session.sendResponse(550, "Failed to read file")
			return
		}
	}

	if session.restartPos > 0 && session.restartPos < int64(len(transferData)) {
		transferData = transferData[session.restartPos:]
		session.restartPos = 0
	}

	bufSize := 16 * 1024 * 1024
	startTime := time.Now()
	for offset := 0; offset < len(transferData); {
		end := offset + bufSize
		if end > len(transferData) {
			end = len(transferData)
		}

		// Validate full data sent
		if len(transferData) == 0 {
			session.sendResponse(451, "No data to transfer")
			return
		} else if offset < len(transferData) {
			session.logger.Printf("Transfer incomplete: only sent %d of %d bytes", offset, len(transferData))
			session.sendResponse(426, "Connection closed before full transfer")
			return
		}
		chunkData := transferData[offset:end]
		for len(chunkData) > 0 {
	...
	if zWriter == nil {
		bufWriter.Flush() // Ensure all data written
	}

	if tcpConn, ok := dataConn.(*net.TCPConn); ok {
		tcpConn.CloseWrite() // force TCP flush
	}

	elapsed := time.Since(startTime)
	throughput := float64(fileSize) / elapsed.Seconds() / 1024 / 1024
	session.logger.Printf("File sent: %s (%d bytes, %.2f MB/s)", filename, fileSize, throughput)
...
func (session *ClientSession) handleSTOR(filename string) {
	...
	// Handle resume support for STOR
	var file *os.File
	...
	if session.restartPos > 0 {
		...
		// Check if restart position is valid
		fileInfo, err := file.Stat()
		if err != nil {
			file.Close()
			session.sendResponse(550, "Failed to get file info for resume")
			return
		}

		if session.restartPos > fileInfo.Size() {
			file.Close()
			session.sendResponse(554, "Restart position beyond file size")
			return
		}
		...
		// Normal mode - create new file with Windows compatibility
		file, err = session.createFileWithRetry(fullPath)
		if err != nil {
			session.sendResponse(550, fmt.Sprintf("Failed to create file: %v", err))
			return
		}
	}
	defer file.Close()
	...
	// Use optimized server STOR handler with large buffers and memory mapping (OPTIMIZATION #2)
	startTime := time.Now()
	...
	session.sendResponse(226, "Transfer complete")
	session.logger.Printf("File received: %s (%d bytes, %.2f MB/s)", filename, bytesWritten, throughput)
...
func (session *ClientSession) handleAPPE(filename string) {
	...
	session.logger.Printf("File appended: %s (%d bytes, %.2f MB/s)", filename, bytesWritten, throughput)
}
